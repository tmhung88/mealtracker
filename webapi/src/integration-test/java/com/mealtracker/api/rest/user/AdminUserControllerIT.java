...
public class AdminUserControllerIT {
    ...
    public void addUser_ValidRequest_ExpectUserAdded() throws Exception {
        mockMvc.perform(post("/v1/users").auth(ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().json("{'data':{'message':'Admin wants to add a new user'}}"));
    }
    ...
    public void listUsers_ValidRequest_ExpectUsersListed() throws Exception {
        mockMvc.perform(get("/v1/users").auth(ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().json("{'data':{'message':'Admin wants to list users'}}"));
    }
    ...
    public void deleteUsers_ValidRequest_ExpectUsersDeleted() throws Exception {
        mockMvc.perform(delete("/v1/users").auth(ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().json("{'data':{'message':'Admin wants to delete users'}}"));
    }
    ...
    public void getUser_ValidRequest_ExpectUserDetailsReturned() throws Exception {
        mockMvc.perform(get("/v1/users/15").auth(ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().json("{'data':{'message':'Admin wants to get details of user 15'}}"));
    }
    ...
    public void updateUser_ValidRequest_ExpectUserDetailsReturned() throws Exception {
        mockMvc.perform(put("/v1/users/2").auth(ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().json("{'data':{'message':'Admin wants to update user 2'}}"));
    }
}
